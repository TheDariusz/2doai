...
/**
 * One real Sonnet round-trip through {@link ProposalPrompt} — the human-in-the-loop check that this
 * slice's prompt produces something worth showing a user. Gated on {@code OPENROUTER_API_KEY} the
 ...
 *
 * <p>The assertion is deliberately thin — a model's prose cannot be pinned without making the test
 * a liar the first time the model improves. What it can hold is the one rule the prompt states as
 * non-negotiable: the message must be about <em>this</em> entry. Everything else is for the human,
 * which is why the output is logged rather than swallowed.
 */
