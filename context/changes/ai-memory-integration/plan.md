...
- [ ] 1.6 `provider` no-training JSON matches OpenRouter's current API (doc cross-check)
- [ ] 1.7 Timeout values sane for a generation call
...
- [ ] 4.1 Hermetic CI green without a key (live test auto-skips)
- [ ] 4.2 Live test passes with a real key — Sonnet free-text + structured (`OpenRouterLiveTest`)

#### Manual

- [ ] 4.3 `fly secrets list` shows `OPENROUTER_API_KEY`; no key value in repo
- [ ] 4.4 Dashboard no-training/no-logging confirmed live; runbook line dated
- [ ] 4.5 Deployed app boots; liveness `UP`; live call against prod config succeeds
- [ ] 4.6 `ai-provider.md` items (a), (b-on-Sonnet), (d) resolved; (c) + Haiku strict deferred to S-09
